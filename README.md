...
- ⏰ **备份频率**: 自定义自动备份频率(1-7天)
- 🗑️ **保留数量**: 保留备份文件数量(默认10个)
- 💾 **立即备份**: 手动创建备份,文件名格式"手动备份-[日期-时间]"
- 📥 **备份导入**: 一键恢复历史备份,数据不丢失
- 📁 **数据管理**: 数据文件统一放在data文件夹,方便管理
- 🔄 **自动备份**: 程序启动时自动检查并执行备份
- 📊 **状态栏**: 底部状态栏显示备份状态和操作提示
- ⚠️ **常请假名单**: 可自定义统计天数(默认5天)和请假次数阈值(默认3次)
- 💾 **设置保存**: 程序关闭自动保存,启动自动加载
...
<details>
<summary><b>v1.0.2 (2025-02-05)</b></summary>

### ✨ 新增
- 实现自动备份功能,程序启动时自动检查并执行备份
- 实现自动删除旧备份功能,每次备份后自动清理
- 添加底部状态栏,显示操作状态和备份信息
- 备份文件名区分手动备份和自动备份
- 自动备份失败时在状态栏显示错误信息
- 添加常请假名单设置,可自定义统计天数和请假次数阈值
- 实现设置保存和加载功能,程序关闭自动保存,启动自动加载

### 🔧 优化
- 手动备份成功后在状态栏显示提示信息
- 保留备份文件数量默认值调整为10个
- 修复窗口图标显示问题(尝试使用PIL加载)
- 自动备份在后台线程执行,避免阻塞UI
- 备份时排除settings.json配置文件
- 导入/删除备份弹窗标题改为"警告"
- 状态栏字体加大(9号→11号)
- 备份列表按文件创建时间排序,自动备份和手动备份混合显示
- 次数阈值不能高于统计天数的限制
...
<details>
<summary><b>v1.0.1 (2025-02-05)</b></summary>
